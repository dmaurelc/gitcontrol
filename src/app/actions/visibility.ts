...
export async function hideOrgAction(login: string) {
...
export async function unhideOrgAction(login: string) {
  const userId = await requireUserId();
  const parsed = orgLoginSchema.parse(login);
  await db
    .update(userPreferences)
...
export async function hideRepoAction(fullName: string) {
  const userId = await requireUserId();
...
export async function unhideRepoAction(fullName: string) {
  const userId = await requireUserId();
  const parsed = fullNameSchema.parse(fullName);
  await db
    .update(userPreferences)
