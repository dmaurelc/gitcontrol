...
export default async function PullsPage({
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title="Pull requests"
        description="Across all your repositories"
      />
      <FilterBar basePath="/pulls" state={state} scope={scope} />
  ...
  if (items.length === 0) {
    return (
      <EmptyState
        ...
        description={`No ${state} PRs match the current filter.`}
      />
    );
  }
      ...
      <Card className="shadow-card">
        <CardContent className="p-0">
          <ul className="divide-y divide-border/60">
            {items.map((it) => (
              <li key={it.id}>
                <IssueRow item={it} kind="pr" />
              </li>
            ))}
          </ul>
        </CardContent>
      </Card>
      <PaginationNav basePath="/pulls" page={page} hasNext={hasNext} />
    </>
  );
}
