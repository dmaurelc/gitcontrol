...
type Props = {
  owner: string;
  repo: string;
  parentPath: string; // current dir path (no trailing slash, "" for root)
...
export function FileTree({ owner, repo, parentPath, entries }: Props) {
      ...
      <CardContent className="p-0">
        <ul className="divide-y divide-border/60">
          {showParent ? (
            <li>
              <Link
                href={
                  parentDir
                    ? `${base}?path=${encodeURIComponent(parentDir)}`
                    : base
                }
          ...
          {sorted.map((entry) => {
            ...
            const href = `${base}?path=${encodeURIComponent(entry.path)}`;
            return (
              <li key={entry.sha}>
                <Link
                  href={href}
